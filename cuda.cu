...
// Función para leer PGM (Soporta P2 y P5)
...
__global__ void smoothingKernel(const int *input, int *output, int width,
                                int height) {
...
int main() {
  string input_file;
  cout << "Ingresa el nombre de la imagen a procesar (ej. fractal.pgm o "
          "ajedrez.pgm): ";
  cin >> input_file;
  ...
  int iterations = 100;
  cout << "Aplicando suavizado (CUDA) sobre " << input_file << " ("
       << iterations << " iteraciones)..." << endl;

  auto start = chrono::high_resolution_clock::now();

  // Ejecutar el kernel
  for (int i = 0; i < iterations; i++) {
    smoothingKernel<<<gridSize, blockSize>>>(d_input, d_output, img.width,
                                             img.height);
    // Swap para la siguiente iteracion (opcional segun logica, aqui output ->
    // input si queremos acumulativo)
